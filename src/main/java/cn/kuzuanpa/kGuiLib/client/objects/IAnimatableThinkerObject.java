...
public interface IAnimatableThinkerObject extends IThinkerObject {
    ...
    default IAnimatableThinkerObject addAnimes(List<IGuiAnime> guiAnimes){
    ...
    static void drawPre(IAnimatableThinkerObject obj, long timer){
        GL11.glPushMatrix();
        obj.getGuiAnimeList().forEach(anime -> anime.animeDrawPre(timer));
    }
    static void draw(IAnimatableThinkerObject obj, long timer){
        obj.getGuiAnimeList().forEach(anime -> anime.animeDraw(timer));
    }
    static void drawAfter(IAnimatableThinkerObject obj, long timer){
        obj.getGuiAnimeList().forEach(anime -> anime.animeDrawPre(timer));
        GL11.glPopMatrix();
    }
    static boolean isAnimeActive(IAnimatableThinkerObject obj, long timer){
